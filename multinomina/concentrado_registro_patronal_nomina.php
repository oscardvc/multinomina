...
//*****************************************************************************************************************
	...
	echo "Concentrado de nomina de trabajadores del $limite_inferior al $limite_superior <br/>Ver totales<input type = 'button' onclick = 'show_totals(this)' value = '⌖' style = 'background:#3399cc;border:none;borderRadius:10px;-moz-border-radius:10px;-webkit-border-radius:10px;color:#fff;text-align:center;cursor:pointer'><br/><br/>";
	...
	$total_diferencia_prima_de_antiguedad = 0;
	...
	while(list($servicio,$trabajador, $registro, $fecha_de_asignacion_del_registro_patronal) = $conn->fetchRow($result))
	{
		...
		$result1 = $conn->query("SELECT Trabajador.Nombre, Trabajador.CURP FROM Trabajador WHERE Trabajador.RFC = '$trabajador' AND Cuenta = '{$_SESSION['cuenta']}'");
		...
		$conn->freeResult($result1);
		//Bajas y reingresos
		$result1 = $conn->query("SELECT Fecha_de_baja, Fecha_de_reingreso FROM Baja WHERE Trabajador = '$trabajador' AND Servicio = '$servicio' AND Cuenta = '{$_SESSION['cuenta']}' AND DATEDIFF('$li',Fecha_de_baja) <= 0 AND DATEDIFF(Fecha_de_baja, '$ls') <= 0");
		$bajas = '';
		...
		$conn->freeResult($result1);
		//Fecha de ingreso
		$result1 = $conn->query("SELECT Antiguedad_IMSS FROM Servicio WHERE id = '$servicio' AND Cuenta = '{$_SESSION['cuenta']}'");
		list($antiguedad) = $conn->fetchRow($result1);
		$conn->freeResult($result1);

		if($antiguedad == 'Servicio')
			$result1 = $conn->query("SELECT Fecha_de_ingreso_servicio FROM Servicio_Trabajador WHERE Trabajador = '$trabajador' AND Servicio = '$servicio' AND Cuenta = '{$_SESSION['cuenta']}'");
		else
			$result1 = $conn->query("SELECT Fecha_de_ingreso_cliente FROM Servicio_Trabajador WHERE Trabajador = '$trabajador' AND Servicio = '$servicio' AND Cuenta = '{$_SESSION['cuenta']}'");

		list($ingreso) = $conn->fetchRow($result1);
		$conn->freeResult($result1);

		if(isset($tipo) && $_GET['solo_asimilados'] == 'no')
		{
			//Vacaciones y prima vacacional
			$result1 = $conn->query("SELECT Vacaciones, Prima_vacacional, Compensacion, ISR, Saldo FROM Recibo_de_vacaciones WHERE DATEDIFF(Fecha, '$li' ) >= 0 AND DATEDIFF(Fecha, '$ls' ) <= 0 AND Trabajador = '$trabajador' AND Cuenta = '{$_SESSION['cuenta']}' ORDER BY Fecha ASC");
			$vacaciones_ = 0;
			$prima_vacacional_ = 0;
...
AND ISRasalariados.Trabajador = '$trabajador' AND nomina_asalariados.Trabajador = '$trabajador' AND Nomina.Servicio = '$servicio' AND Nomina.Cuenta = '{$_SESSION['cuenta']}' AND ISRasalariados.Cuenta = '{$_SESSION['cuenta']}' AND nomina_asalariados.Cuenta = '{$_SESSION['cuenta']}'");
			...
			while(list($nomina, $lip, $lsp, $numero_de_dias_laborados, $sueldo, $subsidio_al_empleo,$subsidio, $horas_extra, $prima_dominical, $dias_de_descanso, $premios_de_puntualidad_y_asistencia, $bonos_de_productividad, $estimulos, $compensaciones, $despensa, $comida, $alimentacion, $habitacion, $aportacion_patronal_al_fondo_de_ahorro, $isr, $impuesto_determinado, $cuotas_imss, $retencion_por_alimentacion, $retencion_por_habitacion, $retencion_infonavit, $retencion_fonacot, $aportacion_del_trabajador_al_fondo_de_ahorro, $pension_alimenticia, $retardos, $saldo) = $conn->fetchRow($result1))
			{
				...
				if($lip < $li || $lsp > $ls)
				{
					...
					$horas_extra = prop($horas_extra, $li, $ls, $lip, $lsp, $trabajador, $tipo, $nomina, $servicio);
...
AND prestaciones.Trabajador = '$trabajador' AND Nomina.Servicio = '$servicio' AND Nomina.Cuenta = '{$_SESSION['cuenta']}' AND prestaciones.Cuenta = '{$_SESSION['cuenta']}'");
			...
			while(list($nomina, $lip, $lsp, $retencion_vacaciones, $retencion_prima_vacacional, $retencion_aguinaldo, $retencion_prima_de_antiguedad, $retencion_prestaciones) = $conn->fetchRow($result1))
			{
			...
			$total_diferencia_vacaciones += $vacaciones_ - $retencion_vacaciones_;
			$total_diferencia_prima_vacacional += $prima_vacacional_ - $retencion_prima_vacacional_;
			$total_diferencia_aguinaldo += $aguinaldo_ordinario_ - $retencion_aguinaldo_;
		...
		$tipo = NULL;
		$result1 = $conn->query("SELECT Tipo FROM Tipo WHERE Trabajador = '$trabajador' AND Tipo = 'Asimilable' AND Servicio = '$servicio' AND Cuenta = '{$_SESSION['cuenta']}' AND DATEDIFF('$ls',Fecha) >= 0 ORDER BY Fecha DESC LIMIT 1");
		list($tipo) = $conn->fetchRow($result1);
		$conn->freeResult($result1);

		if(isset($tipo))
		{
			$result1 = $conn->query("SELECT Tipo FROM Tipo WHERE Trabajador = '$trabajador' AND Tipo = 'Asimilable' AND Servicio = '$servicio' AND Cuenta = '{$_SESSION['cuenta']}' AND DATEDIFF('$ls',Fecha) >= 0 LIMIT 1");
			list($tipo) = $conn->fetchRow($result1);
			$conn->freeResult($result1);

			if(isset($tipo) && $_GET['solo_asalariados'] == 'no')
			{
				//registros de nómina
				$result1 = $conn->query("SELECT Nomina.id, Nomina.Limite_inferior_del_periodo, Nomina.Limite_superior_del_periodo, ISRasimilables.Numero_de_dias_laborados, nomina_asimilables.Honorarios_asimilados, ISRasimilables.Subsidio_al_empleo, ISRasimilables.Subsidio,  ISRasimilables.ISR, ISRasimilables.Impuesto_determinado, nomina_asimilables.Saldo FROM (Nomina LEFT JOIN ISRasimilables ON Nomina.id = ISRasimilables.Nomina) LEFT JOIN nomina_asimilables ON Nomina.id = nomina_asimilables.Nomina WHERE
(
	(
		DATEDIFF(Nomina.Limite_inferior_del_periodo,'$li') >= 0
		AND DATEDIFF(Nomina.Limite_inferior_del_periodo, '$ls') <= 0
...
AND ISRasimilables.Trabajador = '$trabajador' AND nomina_asimilables.Trabajador = '$trabajador' AND Nomina.Servicio = '$servicio' AND Nomina.Cuenta = '{$_SESSION['cuenta']}' AND ISRasimilables.Cuenta = '{$_SESSION['cuenta']}' AND nomina_asimilables.Cuenta = '{$_SESSION['cuenta']}' ORDER BY Nomina.Limite_inferior_del_periodo ASC");
				$numero_de_dias_laborados_ = 0;
				$honorarios_ = 0;
				$subsidio_al_empleo_ = 0;
				$subsidio_ = 0;
				...
				while(list($nomina, $lip, $lsp, $numero_de_dias_laborados, $honorarios, $subsidio_al_empleo, $subsidio, $isr, $impuesto_determinado, $saldo) = $conn->fetchRow($result1))
				{
				...
				if($saldo_nomina_ > 0)
				{
					$txt .= "<tr style = 'text-align:right'><td style = 'text-align:center'>$i</td><td style = 'text-align:center'>$servicio</td><td style = 'text-align:center'>$nombre</td><td>$trabajador</td><td>$curp</td><td>Asimilable</td><td>$bajas</td><td>$ingreso</td><td></td><td></td><td></td><td></td><td></td><td></td><td></td><td></td><td></td><td>$numero_de_dias_laborados_</td><td>" . number_format($honorarios_,2,'.',',') . "</td><td>" . number_format($subsidio_al_empleo_,2,'.',',') . "</td><td>" . number_format($subsidio_,2,'.',',') . "</td><td></td><td></td><td></td><td></td><td></td><td></td><td></td><td></td><td></td><td></td><td></td><td></td><td>" . number_format($isr_nomina_,2,'.',',') . "</td><td>" . number_format($impuesto_determinado_,2,'.',',') . "</td><td></td><td></td><td></td><td></td><td></td><td></td><td></td><td></td><td>" . number_format($saldo_nomina_,2,'.',',') . "</td></tr>";
					$i++;
				}
