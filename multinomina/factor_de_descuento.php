...
//Class Factor_de_descuento definition

	class Factor_de_descuento
	{
		//class properties
		//data
		private $id;
		private $Factor_de_descuento;
		...
		{
			$result = $this->conn->query("SELECT id FROM Factor_de_descuento WHERE Cuenta = '{$_SESSION['cuenta']}' ORDER BY id DESC LIMIT 1");
		...
		{
			$result = $this->conn->query("SELECT * FROM Factor_de_descuento WHERE id = '{$this->id}' AND Cuenta = '{$_SESSION['cuenta']}'");
		...
		public function dbStore($update)//if $update is false it stores a new database register with $this' properties if $update is true it updates all database registers (professedly one) with $this' id
		{
			...
			{
				$this->setID();
				$this->conn->query("INSERT INTO Factor_de_descuento(id, Cuenta) VALUES('{$this->id}', '{$_SESSION['cuenta']}')");
			}

			foreach($this as $key => $value)

				if(isset($this->$key) && $key != 'conn' && $key != 'id')
					$this->conn->query("UPDATE Factor_de_descuento SET $key  = '$value' WHERE id = '{$this->id}' AND Cuenta = '{$_SESSION['cuenta']}'");
		...
		public function dbDelete()//delete this entity from database but id has to be set before
		{
			...
			if(isset($this->id))
				$this->conn->query("DELETE FROM Factor_de_descuento WHERE id = '{$this->id}' AND Cuenta = '{$_SESSION['cuenta']}'");

		}

		public function draw($act)//draws $this Factor_de_descuento. if $act == 'EDIT' or $act == 'ADD' the fields can be edited and the form is submitted. if $act == 'DRAW' the fields can't be edited and the form is not submitted
		{
			...
			echo "<form class = \"show_form\">";
				echo "<fieldset class = \"Datos_fieldset\" style = \"visibility:visible\"\>";
					...
					echo "<label class = \"factor_de_descuento_label\">Factor de descuento</label>";
					echo "<textarea class=\"factor_de_descuento_textarea\" name = \"Factor_de_descuento\" title = \"Factor de descuento\"  " .  ($act == 'EDIT' || $act == 'ADD'? "required=true>":"readonly=true>"). "{$this->Factor_de_descuento}</textarea>";
			...
			if($act == 'EDIT' || $act == 'ADD')
				echo "<img title = \"Guardar\" class = \"submit_button\" onclick = \"_submit('$act',this.parentNode,'Factor_de_descuento')\" />";//_submit() at common_entities.js
