...
//Class Tipo definition
	...
	class Tipo
	{
		...
		{
			$result = $this->conn->query("SELECT id FROM Tipo WHERE Cuenta = '{$_SESSION['cuenta']}' ORDER BY id DESC LIMIT 1");
		...
		{
			$result = $this->conn->query("SELECT * FROM Tipo WHERE id = '{$this->id}' AND Cuenta = '{$_SESSION['cuenta']}'");
		...
		public function dbStore($update)//if $update is false it stores a new database register with $this' properties if $update is true it updates all database registers (professedly one) with $this' id
		{
			...
			{
				$this->setID();
				$this->conn->query("INSERT INTO Tipo(id, Cuenta) VALUES ('{$this->id}', '{$_SESSION['cuenta']}')");
			...
			foreach($this as $key => $value)
				...
				if(isset($this->$key))
				{
					...
					if($key != 'conn' && $key != 'id')
						$this->conn->query("UPDATE Tipo SET $key = '$value' WHERE id = '{$this->id}' AND Cuenta = '{$_SESSION['cuenta']}'");

				}
				elseif($key == 'Servicio')
					$this->conn->query("UPDATE Tipo SET Servicio = NULL WHERE id = '{$this->id}' AND Cuenta = '{$_SESSION['cuenta']}'");
		...
		public function dbDelete()//delete this entity from database but id has to be set before
		{
			...
			if(isset($this->id))
			{
				...
				$actividad->set('Identificadores',"id: {$this->id} Tipo: {$this->Tipo} Fecha: {$this->Fecha} Trabajador: $trabajador Servicio: $notation");
				$actividad->dbStore();
				$this->conn->query("DELETE FROM Tipo WHERE id = '{$this->id}' AND Cuenta = '{$_SESSION['cuenta']}'");
			}

		}

		public function draw($act)//draws $this Tipo. if $act == 'EDIT' or $act == 'ADD' the fields can be edited and the form is submitted. If $act == 'DRAW' the fields can't be edited and the form is not submitted
		{
			...
			echo "<form class = \"show_form\">";
				echo "<fieldset class = \"Datos_fieldset\" style = \"visibility:visible\"\>";
					...
					echo "<label class = \"tipo_label\">Tipo de trabajador</label>";
						...
						if(isset($this->Tipo))
							{
								...
								if($this->Tipo == 'Asalariado')
									echo '<option selected>Asalariado</option>';
								else
									echo '<option>Asalariado</option>';

								if($this->Tipo == 'Asimilable')
									echo '<option selected>Asimilable</option>';
								else
									echo '<option>Asimilable</option>';
				...
				echo "<fieldset class = \"Servicio_fieldset\" style = \"visibility:hidden\"\>";
					...
					echo "<div class = 'options' mode = '$act' dbtable1 = 'Tipo' dbtable2 = 'Servicio' _id = '{$this->id}'></div>";
					echo "<table class = \"search_table\"><tr>";

					for($i=0; $i<4; $i++)

						if($i == 0)
							echo "<td class = 'search_button_cell'><input type='button' value='' class = 'search_button' onmouseover = \"search_button_bright(this)\" onmouseout = \"search_button_opaque(this)\" onclick = \"get_options('$act','Tipo','Servicio','{$this->id}',null,this.parentNode.parentNode.parentNode.parentNode.parentNode)\"/></td><td><input type = 'text' class = 'search_field'  id = 'search_field" . $i . "'/></td>";
						else
			...
			if($act == 'EDIT' || $act == 'ADD')
				echo "<img title = \"Guardar\"class = \"submit_button\" onclick = \"_submit('$act',this.parentNode,'Tipo')\" />";//submit_button_brigth and submit_button_opaque at presentation.js _submit() at common_entities.js
