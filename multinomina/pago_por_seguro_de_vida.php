...
//Class Pago_por_seguro_de_vida definition

	class Pago_por_seguro_de_vida
	{
		//class properties
		//data
		private $id;
		...
		{
			$result = $this->conn->query("SELECT id FROM Pago_por_seguro_de_vida WHERE Cuenta = '{$_SESSION['cuenta']}' ORDER BY id DESC LIMIT 1");
		...
		{
			$result = $this->conn->query("SELECT * FROM Pago_por_seguro_de_vida WHERE id = '{$this->id}' AND Cuenta = '{$_SESSION['cuenta']}'");
		...
		public function dbStore($update)//if $update is false it stores a new database register with $this' properties if $update is true it updates all database registers (professedly one) with $this' id
		{
			...
			{
				$this->setID();
				$this->conn->query("INSERT INTO Pago_por_seguro_de_vida(id, Cuenta) VALUES ('{$this->id}', '{$_SESSION['cuenta']}')");
			}
			...
			foreach($this as $key => $value)
				...
				if(isset($this->$key))
				{
					...
					if($key != 'conn' && $key != 'id')
						$this->conn->query("UPDATE Pago_por_seguro_de_vida SET $key = '$value' WHERE id = '{$this->id}' AND Cuenta = '{$_SESSION['cuenta']}'");

				}
				elseif($key == 'Servicio')
					$this->conn->query("UPDATE Pago_por_seguro_de_vida SET Servicio = NULL WHERE id = '{$this->id}' AND Cuenta = '{$_SESSION['cuenta']}'");
		...
		public function dbDelete()//delete this entity from database but id has to be set before
		{
			...
			if(isset($this->id))
			{
				$this->conn->query("DELETE FROM Pago_por_seguro_de_vida WHERE id = '{$this->id}' AND Cuenta = '{$_SESSION['cuenta']}'");
				$this->conn->query("DELETE FROM Descuento_pendiente WHERE Retencion = 'Pago por seguro de vida' AND id = '{$this->id}' AND Cuenta = '{$_SESSION['cuenta']}'");
			}

		}

		public function draw($act)//draws $this Pago_por_seguro_de_vida. if $act == 'EDIT' or $act == 'ADD' the fields can be edited and the form is submitted. if $act == 'DRAW' the fields can't be edited and the form is not submitted
		{
			echo "<div onclick = \"show('Datos_fieldset',this)\" class = \"datos_tab\" id = \"tab1\">Datos</div>";
			...
			echo "<form class = \"show_form\">";
				...
				echo "<fieldset class = \"Servicio_fieldset\" style = \"visibility:hidden\"\>";
					...
					echo "<div class = 'options' mode = '$act' dbtable1 = 'Pago_por_seguro_de_vida' dbtable2 = 'Servicio' _id = '$this->id'></div>";
					echo "<table class = \"search_table\"><tr>";

					for($i=0; $i<4; $i++)

						if($i == 0)
							echo "<td class = 'search_button_cell'><input type='button' value='' class = 'search_button' onmouseover = \"search_button_bright(this)\" onmouseout = \"search_button_opaque(this)\" onclick = \"get_options('$act','Pago_por_seguro_de_vida','Servicio','$this->id',null,this.parentNode.parentNode.parentNode.parentNode.parentNode)\"/></td><td><input type = 'text' class = 'search_field'  id = 'search_field" . $i . "'/></td>";
						else
			...
			if($act == 'EDIT' || $act == 'ADD')
				echo "<img title = \"Guardar\"class = \"submit_button\" onclick = \"_submit('$act',this.parentNode,'Pago_por_seguro_de_vida')\" />";//_submit() at common_entities.js
