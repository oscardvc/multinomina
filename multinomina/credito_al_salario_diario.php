...
//Class Credito_al_salario_diario definition

	class Credito_al_salario_diario
	{
		//class properties
		//data
		...
		{
			$result = $this->conn->query("SELECT Desde_ingresos_de, Hasta_ingresos_de, Subsidio FROM Credito_al_salario_diario WHERE Ano = '$this->Ano' ORDER BY Desde_ingresos_de ASC");
		...
		public function dbStore($update)//if $update is false it stores new database registers with $this' properties if $update is true it updates all database registers with $this' Ano
		{
			...
			if($update == 'true')
				$this->conn->query("DELETE FROM Credito_al_salario_diario WHERE Ano = '{$this->_Ano}'");
			else
			{
				$result = $this->conn->query("SELECT Ano FROM Credito_al_salario_diario WHERE Ano = '{$this->Ano}'");
			...
			for($i=0; $i<$len; $i++)
				$this->conn->query("INSERT INTO Credito_al_salario_diario(Ano, Desde_ingresos_de, Hasta_ingresos_de, Subsidio) VALUES('{$this->Ano}', {$desdes[$i]}, " . ($hastas[$i] == '' ? "NULL" : $hastas[$i]) . ", {$subsidios[$i]})");
		...
		public function dbDelete()//delete this entity from database but Ano has to be set before
		{
			...
			if(isset($this->Ano))
				$this->conn->query("DELETE FROM Credito_al_salario_diario WHERE Ano = '{$this->Ano}'");

		}

		public function draw($act)//draws $this Credito_al_salario_diario. if $act == 'EDIT' or $act == 'ADD' the fields can be edited and the form is submitted. if $act == 'DRAW' the fields can't be edited and the form is not submitted
		{
			...
			echo "<form class = \"show_form\">";
				echo "<fieldset class = \"Datos_fieldset\" style = \"visibility:visible\">";
					...
					echo "<tr><td class = 'title' colspan = '3'>Crédito al salario diario del año:<textarea title = \"Año\" name = \"Ano\" class=\"ano_textarea\" required = true>{$this->Ano}</textarea></td></tr>";
					echo "<tr>";
			...
			if($act == 'EDIT' || $act == 'ADD')
				echo "<img title = \"Guardar\"class = \"submit_button\" onclick = \"_submit('$act',this.parentNode,'Credito_al_salario_diario')\" />";//_submit() at common_entities.js
