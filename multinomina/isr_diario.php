...
//Class ISR_diario definition
	...
	class ISR_diario
	{
		...
		{
			$result = $this->conn->query("SELECT Limite_inferior, Limite_superior, Cuota_fija, Porcentaje FROM ISR_diario WHERE Ano = '{$this->Ano}' ORDER BY Limite_inferior ASC");
		...
		public function dbStore($update)//if $update is false it stores new database registers with $this' properties if $update is true it updates all database registers with $this' Ano
		{
			...
			if($update == 'true')
				$this->conn->query("DELETE FROM ISR_diario WHERE Ano = '{$this->_Ano}'");
			else
			{
				$result = $this->conn->query("SELECT Ano FROM ISR_diario WHERE Ano = '{$this->Ano}'");
			...
			for($i=0; $i<$len; $i++)
				$this->conn->query("INSERT INTO ISR_diario(Ano, Limite_inferior, Limite_superior, Cuota_fija, Porcentaje) VALUES('{$this->Ano}', {$limites_inferiores[$i]}, " . ($limites_superiores[$i] == '' ? 'NULL' : $limites_superiores[$i]) . ", {$cuotas_fijas[$i]}, {$porcentajes[$i]})");
		...
		public function dbDelete()//delete this entity from database but Ano has to be set before
		{
			...
			if(isset($this->Ano))
				$this->conn->query("DELETE FROM ISR_diario WHERE Ano = '{$this->Ano}'");

		}

		public function draw($act)//draws $this ISR_diario. if $act == 'EDIT' or $act == 'ADD' the fields can be edited and the form is submitted. if $act == 'DRAW' the fields can't be edited and the form is not submitted
		{
			...
			echo "<form class = \"show_form\">";
				echo "<fieldset class = \"Datos_fieldset\" style = \"visibility:visible\">";
					...
					echo "<tr><td class = 'title' colspan = '4'>ISR diario del año:<textarea title = \"Año\" name = \"Ano\" class=\"ano_textarea\" required = true>{$this->Ano}</textarea></td></tr>";
					echo "<tr>";
			...
			if($act == 'EDIT' || $act == 'ADD')
				echo "<img title = \"Guardar\"class = \"submit_button\" onclick = \"_submit('$act',this.parentNode,'ISR_diario')\" />";//_submit() at common_entities.js
