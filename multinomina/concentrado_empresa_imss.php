...
(
	(
		...
		AND
			(
				Baja.Fecha_de_reingreso = '0000-00-00'
				OR
				DATEDIFF(Baja.Fecha_de_reingreso,'$limite_inferior') > 0
			)
	)
	...
	function calculate_dias_de_incapacidad($_limite_inferior, $_limite_superior, $_trabajador, $servicio)
	{
		date_default_timezone_set('America/Mexico_City');
		...
		$result = $conn->query("SELECT Fecha_de_inicio, Fecha_de_termino FROM Incapacidad WHERE Trabajador = '$_trabajador' AND Servicio = '$servicio' AND Cuenta = '{$_SESSION['cuenta']}'");
...
//**************************************************************************************************************************************************
	...
	$result = $conn->query("SELECT Nombre FROM Empresa WHERE RFC = '$empresa' AND Cuenta = '{$_SESSION['cuenta']}'");
	list($nombre_empresa) = $conn->fetchRow($result);
	$conn->freeResult($result);
	echo "<img src = 'images/logo_blanco.jpg'/><br/><br/>";
	echo "Empresa: $nombre_empresa<br/>RFC: $empresa<br/>";
	$result = $conn->query("SELECT Servicio.id, Servicio.Periodicidad_de_la_nomina FROM Servicio LEFT JOIN Servicio_Empresa ON Servicio.id = Servicio_Empresa.Servicio WHERE Servicio.Cuenta = '{$_SESSION['cuenta']}' AND Servicio_Empresa.Cuenta = '{$_SESSION['cuenta']}' AND DATEDIFF('$limite_superior', Servicio_Empresa.Fecha_de_asignacion) >= 0 AND Empresa = '$empresa'" . ($servicio == 'Todos' ? "" : " AND id = '$servicio'"));
	...
	while(list($servicio, $fecha_de_asignacion) = $conn->fetchRow($result))
	{
		...
		//setting upper limit
		$result1 = $conn->query("SELECT Fecha_de_asignacion FROM Servicio_Empresa WHERE Servicio = '$servicio' AND Empresa != '$empresa' AND Cuenta = '{$_SESSION['cuenta']}' AND DATEDIFF(Fecha_de_asignacion, '$fecha_de_asignacion') >= 0 AND DATEDIFF('$limite_superior', Fecha_de_asignacion) >= 0 ORDER BY Fecha_de_asignacion ASC LIMIT 1");
		...
		$result1 = $conn->query("SELECT DISTINCT Trabajador FROM Servicio_Trabajador LEFT JOIN Servicio_Empresa ON Servicio_Trabajador.Servicio = Servicio_Empresa.Servicio WHERE Servicio_Trabajador.Cuenta = '{$_SESSION['cuenta']}' AND Servicio_Empresa.Cuenta = '{$_SESSION['cuenta']}' AND DATEDIFF('$ls', Servicio_Empresa.Fecha_de_asignacion) >= 0 AND Servicio_Empresa.Empresa = '$empresa' AND Servicio_Empresa.Servicio = '$servicio'");

		while(list($trabajador) = $conn->fetchRow($result1))
		{
			$result2 = $conn->query("SELECT Nombre, CURP, Numero_IMSS FROM Trabajador WHERE RFC = '$trabajador' AND Cuenta = '{$_SESSION['cuenta']}'");
			list($nombre,$curp,$numero_imss) = $conn->fetchRow($result2);
			$conn->freeResult($result2);
			$fecha_de_alta_imss = calculate_ingreso($trabajador, $li, $servicio);
			$result2 = $conn->query("SELECT Tipo FROM Tipo WHERE Servicio = '$servicio' AND Trabajador = '$trabajador' AND Tipo = 'Asalariado' AND Cuenta = '{$_SESSION['cuenta']}' AND DATEDIFF('$ls', Fecha) >= 0 LIMIT 1");
			...
			if(isset($tipo))
			{
				...
				//faltas
				$result2 = $conn->query("SELECT ISRasalariados.Faltas FROM Nomina LEFT JOIN ISRasalariados ON Nomina.id = ISRasalariados.Nomina WHERE Nomina.Cuenta = '{$_SESSION['cuenta']}' AND ISRasalariados.Cuenta = '{$_SESSION['cuenta']}' AND
	(
		...
		(DATEDIFF(Nomina.Limite_superior_del_periodo,'$li') >= 0 AND DATEDIFF(Nomina.Limite_superior_del_periodo, '$ls') <= 0)
	...
	) AND ISRasalariados.Trabajador = '$trabajador' AND Nomina.Servicio = '$servicio'");
				$faltas_ = '';

				while(list($faltas) = $conn->fetchRow($result2))
				...
				$numero_de_dias_laborados = count($dias_laborados);
				//Bajas y reingresos
				$result2 = $conn->query("SELECT Fecha_de_baja, Fecha_de_reingreso FROM Baja WHERE Trabajador = '$trabajador' AND Cuenta = '{$_SESSION['cuenta']}' AND DATEDIFF('$li', Fecha_de_baja) <= 0 AND DATEDIFF(Fecha_de_baja, '$ls') <= 0 AND Servicio = '$servicio'");
				$bajas = '';

				while(list($baja, $reingreso) = $conn->fetchRow($result2))
				{
					$bajas .= "Servicio: $servicio<br/>Baja: $baja<br/>Reingreso: $reingreso<br/>";
					$total_bajas ++;
				}
				...
				$result2 = $conn->query("SELECT cuotas_IMSS.Salario_diario_integrado FROM Nomina LEFT JOIN cuotas_IMSS ON Nomina.id = cuotas_IMSS.Nomina WHERE Nomina.Cuenta = '{$_SESSION['cuenta']}' AND cuotas_IMSS.Cuenta = '{$_SESSION['cuenta']}' AND
	(
		(
			DATEDIFF(Nomina.Limite_inferior_del_periodo, '$li') >= 0
			AND
			DATEDIFF(Nomina.Limite_inferior_del_periodo, '$ls') <= 0
		)
		...
		(
			...
			AND
			DATEDIFF(Nomina.Limite_superior_del_periodo, '$ls') <= 0
		)
	) AND cuotas_IMSS.Trabajador = '$trabajador' AND Nomina.Servicio = '$servicio'");
				$total_sdis = '';

				while(list($sdis) = $conn->fetchRow($result2))
				{
					$data = explode(',',$sdis);
					$len = count($data);
				...
				$result2 = $conn->query("SELECT Nomina.id, Nomina.Limite_inferior_del_periodo, Nomina.Limite_superior_del_periodo, cuotas_IMSS.Numero_de_dias_cotizados, cuotas_IMSS.Sueldo_integrado, cuotas_IMSS.Cuota_fija_IMSS, cuotas_IMSS.Exedente_3_SMGDF_patronal, cuotas_IMSS.Exedente_3_SMGDF_obrero, cuotas_IMSS.Prestaciones_en_dinero_obreras, cuotas_IMSS.Prestaciones_en_dinero_patronales, cuotas_IMSS.Gastos_medicos_y_pension_obreros, cuotas_IMSS.Gastos_medicos_y_pension_patronales, cuotas_IMSS.Invalidez_y_vida_obrera, cuotas_IMSS.Invalidez_y_vida_patronal, cuotas_IMSS.Cesantia_y_vejez_obrera, cuotas_IMSS.Cesantia_y_vejez_patronal, cuotas_IMSS.Guarderia, cuotas_IMSS.Retiro, cuotas_IMSS.INFONAVIT, cuotas_IMSS.Riesgo_de_trabajo, cuotas_IMSS.Total_de_cuotas_IMSS_obreras, cuotas_IMSS.Total_de_cuotas_IMSS_patronales FROM Nomina LEFT JOIN cuotas_IMSS ON Nomina.id = cuotas_IMSS.Nomina WHERE Nomina.Servicio = '$servicio'  AND Nomina.Cuenta = '{$_SESSION['cuenta']}' AND cuotas_IMSS.Cuenta = '{$_SESSION['cuenta']}' AND
	(
		(
			DATEDIFF(Nomina.Limite_inferior_del_periodo, '$li') >= 0
			AND
			...
			DATEDIFF(Nomina.Limite_superior_del_periodo, '$ls') <= 0
	...
	) AND cuotas_IMSS.Trabajador = '$trabajador'");
				...
				while(list($nomina, $lip, $lsp, $numero_de_dias_cotizados, $sueldo_integrado, $cuota_fija, $exedente, $cuota_sobre_el_exedente, $prestaciones_en_dinero_obreras, $prestaciones_en_dinero_patronales, $gastos_medicos_y_pension_obreros, $gastos_medicos_y_pension_patronales, $invalidez_y_vida_obrera, $invalidez_y_vida_patronal, $cesantia_y_vejez_obrera, $cesantia_y_vejez_patronal, $guarderia,	$retiro, $infonavit, $riesgo_de_trabajo, $cuotas_obreras, $cuotas_patronales) = $conn->fetchRow($result2))
				{
					...
					if($lip < $li || $lsp > $ls)
					{
						...
						$cesantia_y_vejez_patronal = prop($cesantia_y_vejez_patronal, $li, $ls, $lip, $lsp, $trabajador, $nomina, $servicio);
