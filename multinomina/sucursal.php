	...
	class Sucursal
	{
		...
		public function dbDelete()//delete this entity from database but Nombre and Empresa has to be set before
		{
			...
			if(isset($this->Nombre) && isset($this->Empresa))
			{
				...
				$actividad->set('Identificadores',"Nombre: {$this->Nombre} Empresa: $empresa");
				$actividad->dbStore();
				$this->conn->query("DELETE FROM Sucursal WHERE Nombre = '{$this->Nombre}' AND Empresa = '{$this->Empresa}' AND Cuenta = '{$_SESSION['cuenta']}'");
			}
		...
		public function draw($act)//draws $this Sucursal. if $act == 'EDIT' or $act == 'ADD' the fields can be edited and the form is submitted. if $act == 'DRAW' the fields can't be edited and the form is not submitted
		{
			...
			echo '<form class = "show_form">';
				...
				echo "<fieldset class = \"Logo_fieldset\" style = \"visibility:hidden\"\>";
					echo "<img class = 'logo_image' title = 'Logotipo' " . ($act == 'EDIT' ? "onclick = \"_new('Logo',this)\"" : "") . " sucursal = {$this->Nombre} empresa_sucursal = {$this->Empresa} />";
				echo "</fieldset>";
				echo "<fieldset class =  \"Sello_digital_fieldset\" style = \"visibility:hidden\"\>";
					...
					echo "<div class = 'options' mode = '$act' dbtable1 = 'Sucursal' dbtable2 = 'Sello_digital' _id = '{$this->Nombre}>>{$this->Empresa}'></div>";
					echo "<table class = \"search_table\"><tr>";

					for($i=0; $i<1; $i++)

						if($i == 0)
							echo "<td class = 'search_button_cell'><input type='button' value='' class = 'search_button' onmouseover = \"search_button_bright(this)\" onmouseout = \"search_button_opaque(this)\" onclick = \"get_options('$act','Sucursal','Sello_digital','{$this->Nombre}>>{$this->Empresa}',null,this.parentNode.parentNode.parentNode.parentNode.parentNode)\"/></td><td><input type = 'text' class = 'search_field'  id = 'search_field" . $i . "'/></td>";
						else
							echo "<td><input type = 'text' class = 'search_field'" . $i . "'/></td>";
