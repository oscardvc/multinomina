...
function form_val(_form)
{
	...
	for(var i=0; i<fieldsets.length; i++)
	{
		...
		else if(_title && _title.innerHTML == 'Nuevo salario m√≠nimo')
		{
			var textareas = _form.getElementsByTagName('textarea');
...
function chkRegExp(element)//compares an element value  with its regular expresion
{
	...
	else if(required && (value == ''))
	{
		...
		if(_title)
			...
			_alert('Por favor escriba toda la informaci√≥n necesaria');
	...
	if(values == null)
	{
		_alert('El campo ' + _title + ' no est√° bien escrito');
		return false;
	}
...
function get_code(obj)//sets the "Codigo" for "Salario mÌnimo" obj is textarea
{
