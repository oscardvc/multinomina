...
//Class Prestamo_caja definition
	...
	class Prestamo_caja
	{
		...
		{
			$result = $this->conn->query("SELECT Numero_de_prestamo FROM Prestamo_caja WHERE Cuenta = '{$_SESSION['cuenta']}' ORDER BY Numero_de_prestamo DESC LIMIT 1");
		...
		{
			$result = $this->conn->query("SELECT Numero_de_descuento, Fecha_de_descuento, Cantidad_a_descontar FROM Prestamo_caja WHERE Numero_de_prestamo = '{$this->Numero_de_prestamo}' AND Cuenta = '{$_SESSION['cuenta']}'");
			...
			$result = $this->conn->query("SELECT Trabajador,Servicio FROM Trabajador_Prestamo_caja WHERE Prestamo_caja = '{$this->Numero_de_prestamo}' AND Cuenta = '{$_SESSION['cuenta']}'");
		...
		public function dbStore($update)//if $update is false it stores new database registers with $this' properties if $update is true it updates all database registers with $this' Numero_de_prestamo
		{
			...
			for($i=0; $i<$len; $i++)
				$this->conn->query("INSERT INTO Prestamo_caja(Numero_de_prestamo, Numero_de_descuento, Cuenta)  VALUES({$this->Numero_de_prestamo}, {$numeros_de_descuento[$i]}, '{$_SESSION['cuenta']}')");

			$fechas_de_descuento = explode(',',$this->Fecha_de_descuento);
			$len = count($fechas_de_descuento);

			for($i=1; $i<=$len; $i++)
				$this->conn->query("UPDATE Prestamo_caja SET Fecha_de_descuento = '{$fechas_de_descuento[$i-1]}' WHERE Numero_de_prestamo = {$this->Numero_de_prestamo} AND Numero_de_descuento = $i AND Cuenta = '{$_SESSION['cuenta']}'");

			$cantidades_a_descontar = explode(',',$this->Cantidad_a_descontar);
			$len = count($cantidades_a_descontar);

			for($i=1; $i<=$len; $i++)
				$this->conn->query("UPDATE Prestamo_caja SET Cantidad_a_descontar = {$cantidades_a_descontar[$i-1]} WHERE Numero_de_prestamo = {$this->Numero_de_prestamo} AND Numero_de_descuento = $i AND Cuenta = '{$_SESSION['cuenta']}'");

			$this->conn->query("INSERT INTO Trabajador_Prestamo_caja(Trabajador, Prestamo_caja, Cuenta)  VALUES('{$this->Trabajador}', '{$this->Numero_de_prestamo}', '{$_SESSION['cuenta']}')");

			if(isset($this->Servicio))
				$this->conn->query("UPDATE Trabajador_Prestamo_caja SET Servicio = {$this->Servicio} WHERE Prestamo_caja = {$this->Numero_de_prestamo} AND Cuenta = '{$_SESSION['cuenta']}'");
			else
				$this->conn->query("UPDATE Trabajador_Prestamo_caja SET Servicio = NULL WHERE Prestamo_caja = {$this->Numero_de_prestamo} AND Cuenta = '{$_SESSION['cuenta']}'");
		...
		public function dbDelete($drop = true)//delete this entity from database but Numero_de_prestamo has to be set before
		{
			...
			{
				$this->conn->query("DELETE FROM Prestamo_caja WHERE Numero_de_prestamo = {$this->Numero_de_prestamo} AND Cuenta = '{$_SESSION['cuenta']}'");
				$this->conn->query("DELETE FROM Trabajador_Prestamo_caja WHERE Prestamo_caja = {$this->Numero_de_prestamo} AND Cuenta = '{$_SESSION['cuenta']}'");

				if($drop)
					$this->conn->query("DELETE FROM Descuento_pendiente WHERE Retencion = 'Préstamo caja' AND id = '{$this->Numero_de_prestamo}' AND Cuenta = '{$_SESSION['cuenta']}'");

			}

		}

		public function draw($act)//draws $this Prestamo_caja. if $act == 'EDIT' or $act == 'ADD' the fields can be edited and the form is submitted to store_prestamo_caja.php. if $act == 'DRAW' the fields can't be edited and the form is not submitted
		{
			...
			echo "<form class = \"show_form\">";
				...
				echo "<fieldset class = \"Servicio_fieldset\" style = \"visibility:hidden\"\>";
					...
					echo "<div class = 'options' mode = '$act' dbtable1 = 'Prestamo_caja' dbtable2 = 'Servicio' _id = '$this->Numero_de_prestamo'></div>";
					echo "<table class = \"search_table\"><tr>";

					for($i=0; $i<4; $i++)

						if($i == 0)
							echo "<td class = 'search_button_cell'><input type='button' value='' class = 'search_button' onmouseover = \"search_button_bright(this)\" onmouseout = \"search_button_opaque(this)\" onclick = \"get_options('$act','Prestamo_caja','Servicio','$this->Numero_de_prestamo',null,this.parentNode.parentNode.parentNode.parentNode.parentNode)\"/></td><td><input type = 'text' class = 'search_field'  id = 'search_field" . $i . "'/></td>";
						else
			...
			if($act == 'EDIT' || $act == 'ADD')
				echo "<img title = \"Guardar\"class = \"submit_button\" onclick = \"_submit('$act',this.parentNode,'Prestamo_caja')\" />";//_submit() at common_entities.js
