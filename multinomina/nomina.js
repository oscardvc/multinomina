function get_workers(mode,obj)
{
	...
	for(var i=0; i<textareas.length; i++)
		...
		else if(textareas[i].getAttribute('name') == 'Etapa')
			var etapa = textareas[i].value;
	...
	if(1)//user == 'nomina'
	{
		...
		if(etapa == 'Timbrada')
		{
			_alert('Debe cancelar la nómina para poder editarla');
		}
		else if(fecha_de_pago != '' && limite_inferior != '' && limite_superior != '' && servicio != '')
		{
			var _div = obj.parentNode.parentNode.parentNode;
...
function cancel_nomina(obj)
{
	...
	if (window.XMLHttpRequest)//Mozilla, Safari...
	{
		var xmlcancelnomina = new XMLHttpRequest();

		if (xmlcancelnomina.overrideMimeType)
		{
			xmlcancelnomina.overrideMimeType('text/xml');
		}
	...
	else if (window.ActiveXObject)// IE
	{
		...
		try
		{
			var xmlcancelnomina = new ActiveXObject("Msxml2.XMLHTTP");
		}
		catch (e)
		{
			...
			try
			{
				var xmlcancelnomina = new ActiveXObject("Microsoft.XMLHTTP");
			}
			catch (e) {}
	...
	xmlcancelnomina.onreadystatechange = function()
	{

		if (xmlcancelnomina.readyState==4 && xmlcancelnomina.status==200)
		{

			if(xmlcancelnomina.responseText == '')
			{
				_alert('Listo. revise que todos los CFDI hallan sido cancelados');
				close_div(obj.parentNode.parentNode);
			}
			else
			{
				...
				_alert(xmlcancelnomina.responseText);
			}
	...
	var params = 'nomina=' + nomina;
	xmlcancelnomina.open('POST','cancel_nomina.php', true);
	xmlcancelnomina.setRequestHeader("Content-type", "application/x-www-form-urlencoded; charset=UTF-8");
	xmlcancelnomina.setRequestHeader("cache-control","no-cache");
	xmlcancelnomina.setRequestHeader("Content-length", params.length);
	xmlcancelnomina.setRequestHeader("Connection", "close");
	xmlcancelnomina.send(params);
}

function view_cfdi_prin_nomina(obj)
{
	var textareas = obj.parentNode.getElementsByTagName('textarea');
	...
	window.open('view_cfdi_print_nomina.php?id=' + id, 'Menubar =YES');
}

function download_cfdi_nomina(obj)
{
	var textareas = obj.parentNode.getElementsByTagName('textarea');
	...
	window.location = 'download_cfdi_nomina.php?id=' + id;
}
