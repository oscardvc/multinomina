...
//Class Prestamo_del_fondo_de_ahorro definition

	class Prestamo_del_fondo_de_ahorro
	{
		//class properties
		//data
		...
		{
			$result = $this->conn->query("SELECT Numero_de_prestamo FROM Prestamo_del_fondo_de_ahorro WHERE Cuenta = '{$_SESSION['cuenta']}' ORDER BY Numero_de_prestamo DESC LIMIT 1");
		...
		{
			$result = $this->conn->query("SELECT Numero_de_descuento, Fecha_de_descuento, Cantidad_a_descontar FROM Prestamo_del_fondo_de_ahorro WHERE Numero_de_prestamo = '{$this->Numero_de_prestamo}' AND Cuenta = '{$_SESSION['cuenta']}'");
			...
			$result = $this->conn->query("SELECT Trabajador, Servicio FROM Trabajador_Prestamo_del_fondo_de_ahorro WHERE Prestamo_del_fondo_de_ahorro = '{$this->Numero_de_prestamo}' AND Cuenta = '{$_SESSION['cuenta']}'");
		...
		public function dbStore($update)//if $update is false it stores new database registers with $this' properties if $update is true it updates all database registers with $this' Numero_de_prestamo
		{
			...
			for($i=0; $i<$len; $i++)
			{
				$this->conn->query("INSERT INTO Prestamo_del_fondo_de_ahorro(Numero_de_prestamo, Numero_de_descuento, Cuenta)  VALUES({$this->Numero_de_prestamo}, {$numeros_de_descuento[$i]}, '{$_SESSION['cuenta']}')");
			}
			...
			for($i=1; $i<=$len; $i++)
				$this->conn->query("UPDATE Prestamo_del_fondo_de_ahorro SET Fecha_de_descuento = '{$fechas_de_descuento[$i-1]}' WHERE Numero_de_prestamo = {$this->Numero_de_prestamo} AND Numero_de_descuento = $i AND Cuenta = '{$_SESSION['cuenta']}'");

			$cantidades_a_descontar = explode(',',$this->Cantidad_a_descontar);
			$len = count($cantidades_a_descontar);

			for($i=1; $i<=$len; $i++)
				$this->conn->query("UPDATE Prestamo_del_fondo_de_ahorro SET Cantidad_a_descontar = {$cantidades_a_descontar[$i-1]} WHERE Numero_de_prestamo = {$this->Numero_de_prestamo} AND Numero_de_descuento = $i AND Cuenta = '{$_SESSION['cuenta']}'");

			$this->conn->query("INSERT INTO Trabajador_Prestamo_del_fondo_de_ahorro(Trabajador, Prestamo_del_fondo_de_ahorro, Cuenta) VALUES('{$this->Trabajador}', '{$this->Numero_de_prestamo}', '{$_SESSION['cuenta']}')");

			if(isset($this->Servicio))
				$this->conn->query("UPDATE Trabajador_Prestamo_del_fondo_de_ahorro SET Servicio = '{$this->Servicio}' WHERE Prestamo_del_fondo_de_ahorro = {$this->Numero_de_prestamo} AND Cuenta = '{$_SESSION['cuenta']}'");
			else
				$this->conn->query("UPDATE Trabajador_Prestamo_del_fondo_de_ahorro SET Servicio = NULL WHERE Prestamo_del_fondo_de_ahorro = {$this->Numero_de_prestamo} AND Cuenta = '{$_SESSION['cuenta']}'");
		...
		public function dbDelete($drop = true)//delete this entity from database but Numero_de_prestamo has to be set before
		{
			...
			{
				$this->conn->query("DELETE FROM Prestamo_del_fondo_de_ahorro WHERE Numero_de_prestamo = {$this->Numero_de_prestamo} AND Cuenta = '{$_SESSION['cuenta']}'");
				$this->conn->query("DELETE FROM Trabajador_Prestamo_del_fondo_de_ahorro WHERE Prestamo_del_fondo_de_ahorro = {$this->Numero_de_prestamo} AND Cuenta = '{$_SESSION['cuenta']}'");

				if($drop)
					$this->conn->query("DELETE FROM Descuento_pendiente WHERE Retencion = 'Préstamo del fondo de ahorro' AND id = '{$this->Numero_de_prestamo}' AND Cuenta = '{$_SESSION['cuenta']}'");

			}

		}

		public function draw($act)//draws $this Prestamo_del_fondo_de_ahorro. if $act == 'EDIT' or $act == 'ADD' the fields can be edited and the form is submitted. if $act == 'DRAW' the fields can't be edited and the form is not submitted
		{
			...
			echo "<form class = \"show_form\">";
				echo "<fieldset class = \"Datos_fieldset\" style = \"visibility:visible\">";
					...
					if($act != 'DRAW')
					{
						...
						echo '<td class = "button" onmouseover = "sub_row_button_bright(this)" onmouseout = "sub_row_button_opaque(this)" onclick = "sub_row(this)">━</td>';//sub_row_button_brigth and sub_row_button_opaque at presentation.js. sub_row() at estities.js
					}

					echo "</tr></table>";

					if($act == 'DRAW')
						echo "<img title = 'Ver estado de cuenta' class = 'status_button' onclick = \"estado_de_cuenta(this)\"/>";//estado_de_cuenta at entities.js
				...
				echo "<fieldset class = \"Servicio_fieldset\" style = \"visibility:hidden\"\>";
					...
					echo "<div class = 'options' mode = '$act' dbtable1 = 'Prestamo_del_fondo_de_ahorro' dbtable2 = 'Servicio' _id = '$this->Numero_de_prestamo'></div>";
					echo "<table class = \"search_table\"><tr>";

					for($i=0; $i<4; $i++)

						if($i == 0)
							echo "<td class = 'search_button_cell'><input type='button' value='' class = 'search_button' onmouseover = \"search_button_bright(this)\" onmouseout = \"search_button_opaque(this)\" onclick = \"get_options('$act','Prestamo_del_fondo_de_ahorro','Servicio','$this->Numero_de_prestamo',null,this.parentNode.parentNode.parentNode.parentNode.parentNode)\"/></td><td><input type = 'text' class = 'search_field'  id = 'search_field" . $i . "'/></td>";
						else
			...
			if($act == 'EDIT' || $act == 'ADD')
				echo "<img title = \"Guardar\"class = \"submit_button\" onclick = \"_submit('$act',this.parentNode,'Prestamo_del_fondo_de_ahorro')\" />";//_submit() at common_entities.js
